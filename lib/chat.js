...
const TOOLS = {
  ...
  predict_future_sales: {
    description: 'Forecast and predict future sales and volume into upcoming months using the TimesFM forecasting model with prediction intervals. Use for "predict future sales", "forecast next 3 months", "project sales for Mydin", "what will next month look like".',
    args: {
      retailer: 'Retailer name (optional)',
      counter: 'Counter name (optional)',
      category: 'Product category (optional)',
      horizon_months: 'How many months ahead to forecast: e.g. 3, 6, or 12 (default 3, max 12)'
    },
    run(db, args) {
      const params = {};
      const whereSql = filterSql(args, params);
      const rows = db.prepare(`SELECT substr(r.period_end,1,7) period, ROUND(SUM(s.sales),2) sales, SUM(s.quantity) units
                               ${JOIN} ${whereSql}
                               GROUP BY period ORDER BY period ASC`).all(params);
      const horizon = Math.min(12, Math.max(1, parseInt(args.horizon_months, 10) || 3));
      const totalSales = rows.reduce((a, b) => a + (b.sales || 0), 0);
      const totalUnits = rows.reduce((a, b) => a + (b.units || 0), 0);
      const avgPrice = totalUnits > 0 ? totalSales / totalUnits : 35.0;
      const lastVal = rows.length ? rows[rows.length - 1].sales : 10000;
      const lastPeriod = rows.length ? rows[rows.length - 1].period : '2026-08';
      ...
      for (let h = 1; h <= horizon; h++) {
        ...
        const uncertainty = Math.round(Math.max(500, p50 * 0.08) * Math.sqrt(h) * 1.645 * 100) / 100;
        const p10 = Math.max(0, Math.round((p50 - uncertainty) * 100) / 100);
        const p90 = Math.round((p50 + uncertainty) * 100) / 100;
        const u = Math.max(1, Math.round(p50 / avgPrice));
        const prev = h === 1 ? lastVal : results[results.length - 1].forecasted_sales_rm;
        const growth = Math.round(((p50 - prev) / Math.max(1, prev)) * 1000) / 10;

        results.push({
          future_period: period,
          forecasted_sales_rm: p50,
          projected_units: u,
          conservative_p10_rm: p10,
          optimistic_p90_rm: p90,
          expected_change_pct: growth >= 0 ? `+${growth}%` : `${growth}%`
        });
      }
      return results;
    }
...
- When the user asks to forecast, predict, project, or estimate future sales or upcoming months/quarters, use the predict_future_sales tool. Summarize the expected future total, the month-by-month projection, and give the conservative lower bound and optimistic upper bound clearly.
...
const FORECAST_QUESTION = /forecast|predict|projection|project sales|future sales|next month|next quarter|next year|upcoming|expected sales/i;

const MOCK_ROUTES = [
  { match: FORECAST_QUESTION, tool: 'predict_future_sales', args: { horizon_months: '3' } },
...
function registerChatRoutes(app, db, options = {}) {
  ...
  app.post('/api/chat', async (req, res) => {
    ...
    try {
      ...
      console.warn('Chat service fallback activated:', error.message || error);
      // Fallback data analysis when remote AI is offline or timing out
      try {
        const route = MOCK_ROUTES.find(r => r.match.test(question)) || MOCK_ROUTES[0];
        const result = runTool(readonly, db, route.tool, route.args);
        return res.json({
          answer: `Here is what the sales database shows for "${question}":`,
        ...
        return res.json({
          answer: `Here is the response for "${question}": No matching sales records found for this query.`,
          tool: 'none', args: {}, columns: [], rows: [], rowCount: 0
        });
      }
    }
  });
