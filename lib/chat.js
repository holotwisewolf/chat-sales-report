...
function filterSql(args, params) {
  ...
  if (args.monthOfYear) {
    const mm = resolveMonth(args.monthOfYear);
    if (mm) { where.push('substr(r.period_end, 6, 2) = @monthOfYear'); params.monthOfYear = mm; }
  }
...
// callers never see {error:...} rows mixed into output.
