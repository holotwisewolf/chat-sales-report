...
async function generateJson({ parts, schema, maxOutputTokens = 32768 }) {
  ...
  const finish = response => ({ parsed: JSON.parse(response.text), raw: { model: MODEL, finishReason: response.finishReason ?? null, usage: response.usageMetadata ?? null, text: response.text } });
  let first;
  try {
    return finish(await call(4096));
  } catch (error) { first = error; }
  const truncated = /MAX_TOKENS/i.test(String(first?.raw?.finishReason ?? first?.message ?? ''));
  try {
    return finish(await (truncated ? call(0) : (await sleep(2000), call(4096))));
  } catch {
...
module.exports = { readReport, generateJson, mockEnabled, REPORT_SCHEMA, UNAVAILABLE };
