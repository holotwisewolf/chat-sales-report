...
const QUOTA = 'The daily limit for the AI reader is used up. It resets on its own, usually within a day. Everything else in the app still works.';
const INVALID_KEY = 'The Gemini API key is missing or invalid. Please check the GEMINI_API_KEY in your .env file.';
const NETWORK_ERR = 'Could not connect to Google Gemini. Please check your internet connection and try again.';
...
const mockEnabled = () => process.env.GEMINI_MOCK === '1' || process.env.GEMINI_MOCK === 'empty' || process.env.GEMINI_MOCK === 'fail' || !getApiKey();
...
async function generateRaw({ parts, config = {}, contents = null }) {
  ...
  if (!apiKey) throw fail('invalid_key', INVALID_KEY);
  ...
  try {
    ...
    const errStr = String(first?.message ?? '') + String(first?.code ?? '') + String(first?.status ?? '') + String(first?.cause?.message ?? '');
    const quota = /\b429\b|resource_?exceeded|quota exceeded|rate limit/i.test(errStr);
    if (quota) throw fail('quota', QUOTA);

    const invalidKey = /API_KEY_INVALID|API key not valid|invalid api key|PERMISSION_DENIED|unregistered callers/i.test(errStr);
    if (invalidKey) {
      console.error('[Gemini API Auth Error]', first.message || first);
      throw fail('invalid_key', INVALID_KEY);
    }

    const network = /enotfound|econnrefused|econnreset|fetch failed/i.test(errStr);
    try {
      return await (await sleep(2000), call(2048));
    } catch (second) {
      const secondStr = String(second?.message ?? '') + String(second?.cause?.message ?? '');
      const timedOut = /timeout|aborted|etimedout/i.test(errStr + secondStr);
      console.error('[Gemini API Error]', second.message || first.message || second);
      if (network) throw fail('network_error', NETWORK_ERR);
