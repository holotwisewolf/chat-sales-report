...
const REPORT_SCHEMA = {
  ...
  propertyOrdering: ['document_type', 'retailer_guess', 'period_guess_start', 'period_guess_end', 'printed_totals', 'rows', 'warnings']
};
...
async function callGemini(buffer, mime, thinkingBudget) {
  const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
  const response = await ai.models.generateContent({
    model: MODEL,
    contents: [{ role: 'user', parts: [{ inlineData: { mimeType: mime, data: buffer.toString('base64') } }, { text: PROMPT }] }],
    config: {
...
// One retry: MAX_TOKENS truncation re-runs with thinking off; other failures retry once after a pause.
async function readReport({ buffer, mime, filename }) {
  if (mockEnabled()) return mockRead({ mime, filename });
  let first;
  try {
    return await callGemini(buffer, mime, 4096);
  } catch (error) { first = error; }
  const truncated = /MAX_TOKENS/i.test(String(first?.raw?.finishReason ?? first?.message ?? ''));
  try {
    return await (truncated ? callGemini(buffer, mime, 0) : (await sleep(2000), callGemini(buffer, mime, 4096)));
  } catch {
...
module.exports = { readReport, REPORT_SCHEMA, UNAVAILABLE };
