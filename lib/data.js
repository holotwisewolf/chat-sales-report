...
function registerDataRoutes(app, db, { counterId, categoryId }) {
  ...
  app.get('/api/rows', (req, res) => {
    const { retailer = '', category = '', month = '', from = '', to = '', q = '', sort = 'period', dir = 'desc' } = req.query;
    ...
    if (req.query.monthOfYear) { where.push('substr(r.period_end,6,2) = @monthOfYear'); params.monthOfYear = String(req.query.monthOfYear).padStart(2, '0'); }
    if (from) { where.push('r.period_end >= @from'); params.from = String(from); }
    if (to) { where.push('r.period_start <= @to'); params.to = String(to); }
    // Multi-month: click-selected includes and right-click excludes, as '08,09' lists.
    ...
    // Excel-style totals for the whole filtered set, and which optional columns actually hold data.
  ...
  // Undo for deletes: recreate the reports, then the rows. rows carry _report = index into reports.
  app.post('/api/rows/restore', (req, res) => {
    ...
    const restored = db.transaction(() => {
      const reportIds = reports.map(report => db.prepare('INSERT INTO reports (retailer, period_start, period_end, source_filename, source_type) VALUES (?, ?, ?, ?, ?)')
        .run(report.retailer, report.period_start, report.period_end, report.source_filename || null, report.source_type || 'manual').lastInsertRowid);
      let count = 0;
      ...
      for (const row of rows) {
        const reportId = reportIds[row._report ?? 0] ?? reportIds[0];
        if (!reportId) continue;
        const category = (row.category || 'Uncategorised').trim() || 'Uncategorised';
        insertLine.run(reportId, counterId(row.counter || 'Unknown', row.retailer || 'Unknown'), categoryId(category), category,
          row.productName || null, row.sku || null, num(row.quantity), num(row.sales), num(row.cost), num(row.profit), num(row.margin_percent));
        count++;
      }
      return count;
    })();
