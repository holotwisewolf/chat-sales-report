...
function registerImportRoutes(app, db, { counterId, categoryId, pruneOrphans }) {
  ...
  function rebuildJob(jobId) {
    ...
    if (results.length) {
      ...
      const flags = similarCounterFlags(rows, job.retailer || merged.retailerGuess || '');
      if (flags.length) {
        for (const f of flags) {
          const exist = recon.flagged.find(item => item.index === f.index && item.fileId === f.fileId);
          if (exist) exist.reasons.push(...f.reasons);
          else recon.flagged.push(f);
        }
      }
  ...
  async function processJob(jobId, onlyFileId) {
    try {
      ...
      q.updateJobState.run('failed_read', error.code || 'read_failed', error.message || UNAVAILABLE, null, null, null, jobId);
    }
  }
  ...
  // ---- Spreadsheet intake: CSV / Excel carry structured data already, so no AI read is needed.
  // The browser parses CSV itself; .xlsx/.xls buffers come here for the xlsx package.
  const sheetUpload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 12 * 1024 * 1024 } });
  app.post('/api/parse/sheet', sheetUpload.single('file'), (req, res) => {
    try {
      const file = req.file;
      if (!file) return res.status(400).json({ error: 'No file received.', code: 'no_files' });
      const workbook = XLSX.read(file.buffer, { type: 'buffer' });
      const matrix = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]], { header: 1, raw: true });
      ...
      res.status(400).json({ error: 'We couldn\'t read this spreadsheet. Save it as CSV or a standard .xlsx and try again.', code: 'corrupt_file' });
    }
  });

  // Seed a review job directly from parsed spreadsheet rows - skips the AI entirely and lands
  // in the same review grid (totals checks, near-duplicate flags, overlap-replace on confirm).
  app.post('/api/import-jobs/seed', (req, res) => {
    const body = req.body || {};
    ...
    for (const row of incoming) {
      ...
      if (!String(row.counter || '').trim() || !Number.isFinite(quantity) || !Number.isFinite(sales)) continue;
      rows.push({
        fileId: null, counter: String(row.counter).trim(), sku: row.sku ? String(row.sku).trim() : null,
        product_name: row.productName ? String(row.productName).trim() : null,
        quantity, sales, cost: Number.isFinite(Number(row.cost)) && row.cost !== '' ? Number(row.cost) : null,
        profit: Number.isFinite(Number(row.profit)) && row.profit !== '' ? Number(row.profit) : null,
        margin_percent: null, low_confidence: false, note: null,
        retailer: String(row.retailer || body.retailer || '').trim(),
        category: String(row.category || body.category || '').trim(),
        periodStart: String(row.periodStart || body.periodStart || '').trim(),
        periodEnd: String(row.periodEnd || body.periodEnd || '').trim()
      });
    }
    if (!rows.length) return res.status(400).json({ error: 'No usable rows: each row needs a counter, a quantity, and sales.', code: 'no_rows' });
    const merged = {
      rows, documentType: 'counter_table', retailerGuess: '', periodStart: '', periodEnd: '',
      files: rows.map((row, i) => ({ fileId: null, filename: body.sourceFilename || 'spreadsheet', printedTotals: null, documentType: 'counter_table' })),
      warnings: body.sourceFilename ? [`Imported from spreadsheet ${body.sourceFilename}.`] : ['Imported from spreadsheet.']
    };
    const jobId = q.insertJob.run(null).lastInsertRowid;
    const recon = reconcile(merged, rows);
    const flags = similarCounterFlags(rows, body.retailer || '');
    if (flags.length) {
      for (const f of flags) {
        const exist = recon.flagged.find(item => item.index === f.index);
        if (exist) exist.reasons.push(...f.reasons);
        else recon.flagged.push(f);
      }
    }
    db.prepare("UPDATE import_jobs SET status='review', extracted_json=?, reconciliation_json=?, retailer=?, category=?, period_start=?, period_end=?, updated_at=CURRENT_TIMESTAMP WHERE id=?")
      .run(JSON.stringify(merged), JSON.stringify(recon), body.retailer || null, body.category || null, body.periodStart || null, body.periodEnd || null, jobId);
    res.status(201).json({ job: jobJson(q.job.get(jobId)) });
  });
