...
function registerImportRoutes(app, db, { counterId, categoryId }) {
  ...
  const jobJson = row => {
    ...
    return {
      ...
      duplicateOf: row.status === 'review' && row.retailer && row.period_start && row.period_end && files[0]
        ...
        : null,
      overlaps: row.status === 'review' && row.retailer && row.period_start && row.period_end
        ? q.overlapReports.all(row.retailer, (row.category || '').trim() || 'Uncategorised', row.period_end, row.period_start)
        : [],
      files
    };
  };
  ...
  app.post('/api/import-jobs/:id/confirm', (req, res) => {
    ...
    const run = db.transaction(() => {
      ...
      // A restated window ("1-21 Aug" after "1-18 Aug") replaces the earlier report for the same
      // retailer + category instead of double-counting the shared days. Audit trail survives as 'superseded'.
      const replaced = [];
      if (body.replaceOverlapping !== false) {
        for (const overlap of q.overlapReports.all(retailer, category, periodEnd, periodStart)) {
          ...
          const allMatching = lines.every(line => line.product_category === category);
          if (allMatching) db.prepare('DELETE FROM reports WHERE id = ?').run(overlap.reportId);
          else {
            db.prepare('DELETE FROM sales_lines WHERE report_id = ? AND product_category = ?').run(overlap.reportId, category);
      ...
      return { imported: rows.length, reportId, replaced, totals: recon.overall };
    });
