...
function sanitize(parsed, fileId, filename) {
  ...
  for (const row of parsed.rows || []) {
    ...
    rows.push({
      ...
      // File-level guesses stamped per row so one mixed upload can span several retailers/categories/periods.
      retailer: text(parsed.retailer_guess) || '',
      category: text(parsed.category_guess) || '',
  ...
  return {
    ...
    categoryGuess: text(parsed.category_guess) || '',
    periodStart: text(parsed.period_guess_start) || '',
    periodEnd: text(parsed.period_guess_end) || '',
    filename
  };
